...
-- Table `mydb`.`CLIENTES
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `TOYS`.`CLIENTES` (
...
CREATE TABLE IF NOT EXISTS `TOYS`.`ORDEN_VENTA` (
   ...
   INDEX `FK_CLI_idx` (`CLIENTE`),
   INDEX `FK_SUC_idx` (`SUCURSAL`),
   CONSTRAINT `FK_CLI`
     FOREIGN KEY (`CLIENTE`)
     REFERENCES `TOYS`.`CLIENTES` (`ID_CLIENTE`)
     ON DELETE NO ACTION
     ON UPDATE NO ACTION,
   CONSTRAINT `FK_SUC`
     FOREIGN KEY (`SUCURSAL`)
...
START TRANSACTION;
USE `TOYS`;
INSERT INTO `TOYS`.`CLIENTES` (`NOMBRE`, `DIRECCION`, `TELEFONO`, `CORREO`, `CONTRASEÑA`) VALUES ('Ivan Solorio', NULL, '8713829394', 'ivan@gmail.com', 'ivan12345');
INSERT INTO `TOYS`.`CLIENTES` (`NOMBRE`, `DIRECCION`, `TELEFONO`, `CORREO`, `CONTRASEÑA`) VALUES ('Emiliano Aguilar', NULL, '8714149403', 'emiliano@gmail.com', 'emiliano12345');
...
START TRANSACTION;
USE `TOYS`;
INSERT INTO `TOYS`.`ORDEN_VENTA` (`NO_ORDEN`, `FECHA`, `CLIENTE`, `TIPO`, `HORA`, `FORMA_PAGO`, `SUCURSAL`) VALUES (1, '2023-06-13', 1, 'LLEVAR', '07:27:00', 'EFECTIVO', 1);
INSERT INTO `TOYS`.`ORDEN_VENTA` (`FECHA`, `CLIENTE`, `TIPO`, `HORA`, `FORMA_PAGO`, `SUCURSAL`) VALUES ('2023-06-20', 2, 'LLEVAR', '15:27:00', 'EFECTIVO', 1);
...
SELECT PRODUCTOS.PRECIO, PRODUCTOS.NOMBRE, CLIENTES.NOMBRE FROM PRODUCTOS
JOIN DETALLE_ORDEN ON DETALLE_ORDEN.PRODUCTO = PRODUCTOS.CODIGO
JOIN ORDEN_VENTA ON ORDEN_VENTA.NO_ORDEN = DETALLE_ORDEN.NO_ORDEN
JOIN CLIENTES ON CLIENTES.ID_CLIENTE = ORDEN_VENTA.CLIENTE
WHERE ORDEN_VENTA.NO_ORDEN = 2;

DELIMITER //
CREATE PROCEDURE obtener_cliente(IN cliente_id INT)
BEGIN
    SELECT NOMBRE, TELEFONO, CORREO FROM CLIENTES WHERE ID_CLIENTE = cliente_id;
END //
DELIMITER ;
...
DELIMITER //
CREATE PROCEDURE InsertarCliente(IN c_Nombre VARCHAR(50),
    IN c_Direccion VARCHAR(60),
    IN c_Telefono CHAR(10),
    IN c_Correo VARCHAR(60),
    IN c_Contrasena VARCHAR(50)
)
BEGIN
    INSERT INTO `TOYS`.`CLIENTES` (`NOMBRE`,`DIRECCION`,`TELEFONO`,`CORREO`,`CONTRASEÑA`) VALUES (c_Nombre,
        c_Direccion,
        c_Telefono,
        c_Correo,
        c_Contrasena
...
CALL InsertarCliente('Kasandra Martinez', 'Universidad Tecnologica de Torreon', '8719392002', 'kasandra@gmail.com', 'kassy12345');
